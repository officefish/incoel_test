...
public:
    ...
    ENDPOINT_INFO(info) {
        info->summary = "Получить статус сервера";
    }
    ...
    ENDPOINT_INFO(getLog) {
        info->summary = "Получить логи сервера";
    }
    ...
    ENDPOINT_INFO(uploadFile) {
        info->summary = "Загрузить файл";
        info->addConsumes<String>("multipart/form-data");
    }
...
void run() {
    ...
    logService->addLog("Сервер запущен на порту 1616 с поддержкой Swagger");
    server.run();
